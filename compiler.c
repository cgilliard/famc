...
void node_print_impl(struct node *n, long depth) {
	...
	if (n->kind == nk_asm) {
		...
		write_str(1, " (compound statement)");
...
void proc_nk_asm_complete(struct parser *p) {
	...
	if (p->sp > 1) print_error(&p->stack[p->sp - 2], "unexpected token1");
...
void proc_function(struct parser *p) {
	...
	p->sp--;
	if (p->sp <= 0) print_error(&p->stack[0], "unexpected token6");
	if (p->stack[p->sp].kind != nk_ident)
		print_error(&p->stack[p->sp], "expected identifier");
	arena_alloc((void *)&s, p->a, sizeof(struct slice));
	s->ptr = p->in + p->stack[p->sp].loc.off;
	...
	if (p->sp != 0 || p->stack[0].kind != nk_void)
		print_error(&p->stack[0], "unexpected token7");

	function_node->node_data = s;
	node_append(p->root, function_node, 0);
	p->current = function_node;

	arena_alloc((void *)&cs, p->a, sizeof(struct node));
	node_init(cs, nk_compound_stmt);
	node_append(function_node, cs, 0);
	p->current = cs;
}
...
void proc_break(struct parser *p) {
	...
	arena_alloc((void *)&brk, p->a, sizeof(struct node));
	node_init(brk, nk_break);
	node_append(p->current, brk, 0);
}
...
void proc_goto(struct parser *p) {
	...
	arena_alloc((void *)&gt, p->a, sizeof(struct node));
	node_init(gt, nk_goto);
	gt->node_data = s;
	node_append(p->current, gt, 0);
	p->sp--;
}

void proc_compound_semi(struct parser *p) {
	/*dump_stack(p);*/
	p->sp -= 2;
	if (p->sp >= 0 && p->stack[p->sp].kind == nk_break)
		proc_break(p);
	else {
		if (p->sp > 0 && p->stack[p->sp - 1].kind == nk_goto)
			proc_goto(p);
	}
	p->sp = 0;
}
...
void proc_nk_left_brace_function(struct parser *p) {
	struct node *cs;
	arena_alloc((void *)&cs, p->a, sizeof(struct node));
	node_init(cs, nk_compound_stmt);
...
void proc_nk_semi(struct parser *p) {
	if (p->current == p->root)
		p->sp = 0;
	else if (p->current->kind == nk_compound_stmt)
		proc_compound_semi(p);
}
