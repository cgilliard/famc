...
#define NODE_IS(valptr, tysuf) \
	(dynamic_cast<const ::HIR::ExprNode##tysuf*>(&*valptr) != nullptr)
...
class TreeVisitor : public ::HIR::Visitor, public ::HIR::ExprVisitor {
	...
	unsigned int m_indent_level;
       ...
       public:
	TreeVisitor(::std::ostream& os) : m_os(os), m_indent_level(0) {}
	...
	void visit_type_impl(::HIR::TypeImpl& impl) override {
		m_os << indent() << "impl" << impl.m_params.fmt_args() << " "
		     << impl.m_type << "\n";
	...
	virtual void visit_trait_impl(const ::HIR::SimplePath& trait_path,
				      ::HIR::TraitImpl& impl) override {
		m_os << indent() << "impl" << impl.m_params.fmt_args() << " "
		     << trait_path << impl.m_trait_args << " for "
		     << impl.m_type << "\n";
		if (!impl.m_params.m_bounds.empty()) {
			m_os << indent() << " " << impl.m_params.fmt_bounds()
			     << "\n";
	...
	void visit_type_alias(::HIR::ItemPath p,
		...
		m_os << indent() << "type " << p.get_name()
		     << item.m_params.fmt_args() << " = " << item.m_type
		     << item.m_params.fmt_bounds() << "\n";
	}
	void visit_trait(::HIR::ItemPath p, ::HIR::Trait& item) override {
		m_os << indent() << "trait " << p.get_name()
		     << item.m_params.fmt_args() << " : " << item.m_lifetime
		     << "\n";
	...
	void visit_struct(::HIR::ItemPath p, ::HIR::Struct& item) override {
		m_os << indent() << "struct " << p.get_name()
		     << item.m_params.fmt_args();
	    TU_MATCH_HDRA( (item.m_data), {)
			...
			TU_ARMA(Named, flds) {
				...
				inc_indent();
				for (const auto& fld : flds) {
					m_os << indent() << fld.second.publicity
					     << " " << fld.first << ": "
					     << fld.second.ent << ",\n";
				}
				dec_indent();
	...
	void visit_function(::HIR::ItemPath p, ::HIR::Function& item) override {
		m_os << indent();
		if (item.m_const) m_os << "const ";
		if (item.m_unsafe) m_os << "unsafe ";
		...
		if (item.m_code) {
			m_os << indent();
			if (dynamic_cast< ::HIR::ExprNode_Block*>(
				&*item.m_code)) {
	...
	void visit_static(::HIR::ItemPath p, ::HIR::Static& item) override {
		if (item.m_linkage.name != "")
			m_os << indent() << "#[link_name=\""
			     << item.m_linkage.name << "\"]\n";
		if (item.m_value) {
			m_os << indent() << "static " << p.get_name()
			     << item.m_params.fmt_args() << ": " << item.m_type
			     << " = " << item.m_value_res;
		} else if (item.m_value_generated) {
			m_os << indent() << "static " << p.get_name()
			     << item.m_params.fmt_args() << ": " << item.m_type
			     << " = /*magic*/ " << item.m_value_res;
		} else {
			m_os << indent() << "extern static " << p.get_name()
			     << ": " << item.m_type;
		}
		if (!item.m_params.m_bounds.empty()) {
			m_os << indent() << " " << item.m_params.fmt_bounds()
	...
	void visit_constant(::HIR::ItemPath p, ::HIR::Constant& item) override {
		m_os << indent() << "const " << p.get_name() << ": "
		     << item.m_type << " = " << item.m_value_res;
...
// - Misc
	...
	bool node_is_leaf(const ::HIR::ExprNode& node) {
		if (NODE_IS(&node, _PathValue)) return true;
		if (NODE_IS(&node, _Variable)) return true;
		if (NODE_IS(&node, _Literal)) return true;
		if (NODE_IS(&node, _CallPath)) return true;
		if (NODE_IS(&node, _Deref)) return true;
		return false;
	}
	...
	void visit(::HIR::ExprNode_Match& node) override {
		...
		for (/*const*/ auto& arm : node.m_arms) {
			...
			for (unsigned int i = 1; i < arm.m_patterns.size();
			     i++) {
	...
	void visit(::HIR::ExprNode_Emplace& node) override {
		...
		m_os << ")";
		m_os << "/*"
		     << (node.m_type == ::HIR::ExprNode_Emplace::Type::Boxer
			     ? "box"
			     : "place")
		     << "*/";
	}
	...
	void visit(::HIR::ExprNode_Literal& node) override {
		TU_MATCHA(
		    ...
		    (Boolean, m_os << (e ? "true" : "false");),
		    (String, m_os << "\"" << FmtEscaped(e) << "\"";),
		    (
			ByteString, m_os << "b\""; for (auto b
							: e) {
				if (b == '\\' || b == '\"') {
					m_os << "\\" << b;
				} else if (' ' <= b && b <= 0x7F) {
	...
	void visit(::HIR::ExprNode_Generator& node) override {
		if (node.m_code) {
			m_os << "/*gen*/";
			if (node.m_is_pinned) m_os << "static ";
			if (node.m_is_move) m_os << " move";
			m_os << "|";
			// for(const auto& arg : node.m_args)
			//     m_os << arg.first << ": " << arg.second << ", ";
