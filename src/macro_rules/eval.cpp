...
typedef std::map<unsigned, std::map<std::vector<unsigned>, unsigned>>
    loop_counts_t;
...
class ParameterMappings {
	...
	TAGGED_UNION(CaptureLayer, Vals, (Vals, ::std::vector<CapturedVal>),
	...
	unsigned m_layer_count;
       ...
       public:
	ParameterMappings() : m_layer_count(0) {}
	...
	void dump() const { DEBUG("m_mappings = {" << m_mappings << "}"); }

	size_t layer_count() const { return m_layer_count + 1; }
	...
	void insert(unsigned int name_index,
		    const ::std::vector<unsigned int>& iterations,
		    InterpolatedFragment data);

	/// <summary>
	/// Get the replacement fragment for a given loop iteration (or
	/// `nullptr`) if out of bounds
	/// </summary>
	InterpolatedFragment* get(const Span& sp,
				  const ::std::vector<unsigned int>& iterations,
				  unsigned int name_idx);

	/// <summary>
	/// Given a current iteration and a loop index, return how many times
	/// this loop will run
	/// </summary>
	unsigned int get_loop_repeats(
	    const Span& sp, const ::std::vector<unsigned int>& iterations,
	    unsigned int loop_idx) const;

	/// <summary>
	/// Return the number of times this level of a given name/variable will
	/// loop
	/// </summary>
	unsigned int get_variable_count(
	    const Span& sp, const ::std::vector<unsigned int>& iterations,
	    unsigned int name_idx) const;

	/// Increment the number of times a particular fragment will be used
	void inc_count(const Span& sp,
		       const ::std::vector<unsigned int>& iterations,
		       unsigned int name_idx);
	/// Decrement the number of times a particular fragment is used (returns
	/// true if there are still usages remaining)
	bool dec_count(const Span& sp,
		       const ::std::vector<unsigned int>& iterations,
		       unsigned int name_idx);
	...
	friend ::std::ostream& operator<<(::std::ostream& os,
					  const CaptureLayer& x) {
		TU_MATCH(CaptureLayer, (x), (e), (Vals, os << "[" << e << "]";),
			 (Nested, os << "{" << e << "}";))
		return os;
	}

       private:
	CapturedVal& get_cap(const Span& sp,
			     const ::std::vector<unsigned int>& iterations,
			     unsigned int name_idx);
};
...
class MacroPatternStream {
	...
	const SimplePatEnt* m_peek_cache;
       ...
       public:
	MacroPatternStream(
	    const ::std::vector<SimplePatEnt>& ents,
	    const ::std::vector<bool>* condition_replay = nullptr)
	    : m_simple_ents(ents),
	      m_cur_pos(0),
	...
	/// Inform the stream that the `if` rule that was just returned
	/// succeeded
	void if_succeeded();
	...
	loop_counts_t take_loop_counts() { return ::std::move(m_loop_counts); }
};

// === Prototypes ===
unsigned int Macro_InvokeRules_MatchPattern(
    const Span& sp, const MacroRules& rules, TokenTree input,
    const AST::Crate& crate, AST::Module& mod, ParameterMappings& bound_tts);
void Macro_InvokeRules_CountSubstUses(
...
void ParameterMappings::insert(unsigned int name_index,
			       const ::std::vector<unsigned int>& iterations,
			       InterpolatedFragment data) {
	DEBUG("index=" << name_index << ", iterations=[" << iterations
		       << "], data=" << data);
	...
	if (iterations.size() > 0) {
		for (unsigned int i = 0; i < iterations.size() - 1; i++) {
			...
			if (e.size() == iter) {
				...
				if (e.size() > iter) {
					DEBUG(
					    "ERROR: Iterations ran backwards? "
					    "- "
					    << e.size() << " > " << iter);
				}
			}
		...
		ASSERT_BUG(
		    Span(), layer->as_Vals().size() == iterations.back(),
		    "Capture count mismatch with iteration index - iterations=["
			<< iterations << "]");
	} else {
		assert(layer->as_Vals().size() == 0);
	}
...
ParameterMappings::CapturedVal& ParameterMappings::get_cap(
    const Span& sp, const ::std::vector<unsigned int>& iterations,
    unsigned int name_idx) {
	...
	if (auto* e = layer->opt_Vals()) {
		...
		if (e->size() == 0) {
			BUG(sp,
			    "Attempting to get binding for empty capture - #"
				<< name_idx);
		}
	}
	...
	TU_MATCH_HDRA( (*layer), { )
        TU_ARMA(Vals, e) {
				ASSERT_BUG(sp, iter < e.size(),
					   "Iteration index "
					       << iter << " outside of range "
					       << e.size() << " (values)");
				return e.at(iter);
			}
			TU_ARMA(Nested, e) {
	...
	ERROR(sp, E0000,
	      "Variable #" << name_idx << " is still repeating at this level ("
			   << iterations.size() << ")");
}

InterpolatedFragment* ParameterMappings::get(
    const Span& sp, const ::std::vector<unsigned int>& iterations,
    unsigned int name_idx) {
	return &get_cap(sp, iterations, name_idx).frag;
}
unsigned int ParameterMappings::get_loop_repeats(
    const Span& sp, const ::std::vector<unsigned int>& iterations,
    unsigned int loop_idx) const {
	const auto& list = m_loop_counts.at(loop_idx);
	// Iterate the list, find the first prefix match of `iterations`
	// - `iterations` should always be longer or equal in length to every
	// entry in `list`
	// auto ranges = list.equal_range(iterations);
	for (const auto& e : list) {
		ASSERT_BUG(Span(), e.first.size() <= iterations.size(),
			   "Loop " << loop_idx << " iteration path [" << e.first
				   << "] larger than query path [" << iterations
				   << "]");
		if (std::equal(e.first.begin(), e.first.end(),
			       iterations.begin())) {
			return e.second;
		}
	}
	BUG(sp, "Loop " << loop_idx
			<< " cannot find an iteration count for path ["
			<< iterations << "]");
}
unsigned int ParameterMappings::get_variable_count(
    const Span& sp, const ::std::vector<unsigned int>& iterations,
    unsigned int name_idx) const {
	DEBUG("(iterations=[" << iterations << "], name_idx=" << name_idx
			      << ")");
    ...
    TU_ARMA(Vals, e) {
		...
		TU_ARMA(Nested, e) { return e.size(); }
    }
    throw "";
}

void ParameterMappings::inc_count(const Span& sp,
				  const ::std::vector<unsigned int>& iterations,
				  unsigned int name_idx) {
...
bool ParameterMappings::dec_count(const Span& sp,
				  const ::std::vector<unsigned int>& iterations,
				  unsigned int name_idx) {
	auto& cap = get_cap(sp, iterations, name_idx);
...
const SimplePatEnt& MacroPatternStream::next() {
	...
	for (;;) {
		// If not replaying, and the previous entry was a conditional,
		// record the result of that conditional
		...
		if (m_condition_replay && cur_ent.is_If()) {
			// Skip the conditional (following its target or just
			// skipping over)
	...
	TU_MATCH_HDRA( (cur_ent), {)
        default:
			...
			TU_ARMA(LoopNext, _e) { m_loop_iterations.back() += 1; }
			TU_ARMA(LoopEnd, _e) {
				...
				if (m_condition_replay) {
					m_loop_counts[loop_index].insert(
					    std::make_pair(m_loop_iterations,
							   num_iter));
				}
			}
        }
...
void MacroPatternStream::if_succeeded() {
	...
	ASSERT_BUG(Span(), ent.is_If(),
		   "Expected If when calling `if_succeeded`, got " << ent);
	const auto& e = ent.as_If();
	ASSERT_BUG(Span(), e.jump_target < m_simple_ents.size(),
		   "Jump target " << e.jump_target << " out of range "
				  << m_simple_ents.size());
	m_cur_pos = e.jump_target;
...
class MacroExpandState {
	...
	const ::std::vector<MacroExpansionEnt>*
	    m_cur_ents;	 // For faster lookup.

       public:
	MacroExpandState(const ::std::vector<MacroExpansionEnt>& contents,
			 const ParameterMappings& mappings)
	    : m_root_contents(contents),
	      m_mappings(mappings),
	      m_offsets({{0, 0, 0}}),
	      m_cur_ents(&m_root_contents) {}

	// Returns a pointer to the next entry to expand, or nullptr if the end
	// is reached
	// - NOTE: When a Loop entry is returned, the separator token should be
	// emitted
...
class MacroExpander : public TokenStream {
	...
	Token
	    m_next_token;  // used for inserting a single token into the stream
	...
	MacroExpander(const RcString& macro_name, const Span& sp,
		      AST::Edition edition, bool is_macro_item,
		      const Ident::Hygiene& parent_hygiene,
		      const ::std::vector<MacroExpansionEnt>& contents,
		      ParameterMappings mappings, RcString crate_name,
...
InterpolatedFragment Macro_HandlePatternCap(TokenStream& lex,
					    MacroPatEnt::Type type) {
	Token tok;
	switch (type) {
		case MacroPatEnt::PAT_TOKEN:
			BUG(lex.point_span(),
			    "Encountered PAT_TOKEN when handling capture");
		case MacroPatEnt::PAT_LOOP:
			BUG(lex.point_span(),
			    "Encountered PAT_LOOP when handling capture");
		...
		case MacroPatEnt::PAT_PAT:
			// TODO: Is this edition check correct? Or should it be
			// uncondiitonally "Yes"?
			// return InterpolatedFragment( Parse_Pattern(lex,
			// lex.edition_after(AST::Edition::Rust2021) ?
			// AllowOrPattern::Yes : AllowOrPattern::No) );
			return InterpolatedFragment(
			    Parse_Pattern(lex, AllowOrPattern::Yes));
		case MacroPatEnt::PAT_TYPE:
			return InterpolatedFragment(Parse_Type(lex));
		case MacroPatEnt::PAT_EXPR:
			return InterpolatedFragment(InterpolatedFragment::EXPR,
						    Parse_Expr0(lex).release());
		case MacroPatEnt::PAT_STMT:
			return InterpolatedFragment(InterpolatedFragment::STMT,
						    Parse_Stmt(lex).release());
		case MacroPatEnt::PAT_PATH:
			return InterpolatedFragment(Parse_Path(
			    lex, PATH_GENERIC_TYPE));  // non-expr mode
		case MacroPatEnt::PAT_BLOCK:
			return InterpolatedFragment(
			    InterpolatedFragment::BLOCK,
			    Parse_ExprBlockNode(lex).release());
		...
		case MacroPatEnt::PAT_ITEM: {
			...
			return InterpolatedFragment(Parse_Mod_Item_S(
			    lex, cur_mod.m_file_info, cur_mod.path(),
			    AST::AttributeList{}));
		} break;
		case MacroPatEnt::PAT_IDENT:
			...
			if (Token::type_is_rword(tok.type()))
				return InterpolatedFragment(TokenTree(
				    lex.get_edition(), lex.get_hygiene(), tok));
			else {
		...
		case MacroPatEnt::PAT_VIS:
			return InterpolatedFragment(
			    Parse_Publicity(lex, /*allow_restricted=*/true));
		...
		case MacroPatEnt::PAT_LITERAL:
			...
			if (tok.type() == TOK_DASH) {
				...
				toks.push_back(tok);
				return InterpolatedFragment(TokenTree(
				    lex.get_edition(), lex.get_hygiene(),
				    std::move(toks)));
			}
			switch (tok.type()) {
				case TOK_INTEGER:
				...
				default:
					throw ParseError::Unexpected(
					    lex, tok,
					    {TOK_INTEGER, TOK_FLOAT, TOK_STRING,
					     TOK_BYTESTRING, TOK_RWORD_TRUE,
					     TOK_RWORD_FALSE});
			}
			return InterpolatedFragment(TokenTree(
			    lex.get_edition(), lex.get_hygiene(), tok));
...
/// Parse the input TokenTree according to the `macro_rules!` patterns and
/// return a token stream of the replacement
::std::unique_ptr<TokenStream> Macro_InvokeRules(
    const RcString& name, const MacroRules& rules, const Span& sp,
    TokenTree input, const AST::Crate& crate, AST::Module& mod) {
	TRACE_FUNCTION_F("'" << name << "', " << input);
	DEBUG("rules.m_source_crate = " << rules.m_source_crate);
	DEBUG("rules.m_hygiene = " << rules.m_hygiene);

	ParameterMappings bound_tts;
	unsigned int rule_index = Macro_InvokeRules_MatchPattern(
	    sp, rules, mv$(input), crate, mod, bound_tts);

	const auto& rule = rules.m_rules.at(rule_index);

	DEBUG("Using macro '"
	      << name << "' #" << rule_index << " - " << rule.m_contents.size()
	      << " rule contents with " << bound_tts.mappings().size()
	      << " bound values");
	for (unsigned int i = 0; i < ::std::min(bound_tts.mappings().size(),
						rule.m_param_names.size());
	     i++) {
		DEBUG("- #" << i << " " << rule.m_param_names.at(i) << " = ["
			    << bound_tts.mappings()[i] << "]");
	}
	// bound_tts.dump();

	// Run through the expansion counting the number of times each fragment
	// is used
	Macro_InvokeRules_CountSubstUses(bound_tts, rule.m_contents);

	TokenStream* ret_ptr = new MacroExpander(
	    name, sp, crate.m_edition, rules.m_is_macro_item, rules.m_hygiene,
	    rule.m_contents, mv$(bound_tts),
	    rules.m_source_crate == "" ? crate.m_crate_name_real
				       : rules.m_source_crate,
	    rules.m_edition);

	return ::std::unique_ptr<TokenStream>(ret_ptr);
}

// Collection of functions that consume a specific fragment type from a token
// stream
...
class TokenStreamRO {
	...
	TokenStreamRO(const TokenTree& tt)
	    : m_tt(tt), m_active_offset(0), m_consume_count(0) {
	...
	TokenStreamRO clone() const { return TokenStreamRO(*this); }

	enum eTokenType next() const { return next_tok().type(); }
	const Token& next_tok() const {
		...
		if (m_offsets.empty() && m_active_offset == m_tt.size()) {
			// DEBUG(m_consume_count << " " << eof_token <<
			// "(EOF)");
			return eof_token;
		} else {
			const auto* cur_tree = &m_tt;
			for (auto idx : m_offsets) cur_tree = &(*cur_tree)[idx];
	...
	void consume() {
		...
		for (;;) {
			...
			if (m_active_offset == cur_tree->size()) {
				// If the end of the root is reached, return
				// (leaving the state indicating EOS)
				...
				while (
				    !(*cur_tree)[m_active_offset].is_token()) {
					cur_tree =
					    &(*cur_tree)[m_active_offset];
	...
	// Consumes if the current token is `ty`, otherwise doesn't and returns
	// false
	...
	/// Returns the position in the stream (number of tokens that have been
	/// consumed)
	size_t position() const { return m_consume_count; }
};
...
bool consume_tt(TokenStreamRO& lex) {
	...
	switch (lex.next()) {
		...
		case TOK_PAREN_OPEN:
			lex.consume();
			while (lex.next() != TOK_PAREN_CLOSE) consume_tt(lex);
			lex.consume();
			break;
		case TOK_SQUARE_OPEN:
			lex.consume();
			while (lex.next() != TOK_SQUARE_CLOSE) consume_tt(lex);
			lex.consume();
			break;
		case TOK_BRACE_OPEN:
			lex.consume();
			while (lex.next() != TOK_BRACE_CLOSE) consume_tt(lex);
			lex.consume();
			break;
		default:
			lex.consume();
...
bool consume_tt_angle(TokenStreamRO& lex) {
	...
	for (;;) {
		...
		} else if (lex.next() == TOK_GT ||
			   lex.next() == TOK_DOUBLE_GT) {
			assert(level > 0);
...
bool consume_path(TokenStreamRO& lex, bool type_mode = false) {
	...
	switch (lex.next()) {
		...
		case TOK_RWORD_SELF:
			lex.consume();
			// Allow a lone `self` (it's referring to the current
			// object)
			if (lex.next() != TOK_DOUBLE_COLON) return true;
			break;
		...
		case TOK_RWORD_SUPER:
			lex.consume();
			if (lex.next() != TOK_DOUBLE_COLON) return false;
			break;
		case TOK_DOUBLE_COLON:
			break;
		case TOK_IDENT:
			lex.consume();
			if (type_mode && (lex.next() == TOK_LT ||
					  lex.next() == TOK_DOUBLE_LT ||
					  lex.next() == TOK_PAREN_OPEN))
				;
			// Allow a lone ident
		...
		case TOK_LT:
		case TOK_DOUBLE_LT:
			if (!consume_tt_angle(lex)) return false;
			if (lex.next() != TOK_DOUBLE_COLON) return false;
			break;
		default:
			return false;
	...
	if (type_mode &&
	    (lex.next() == TOK_LT || lex.next() == TOK_DOUBLE_LT)) {
		if (!consume_tt_angle(lex)) return false;
	}
	...
	while (lex.next() == TOK_DOUBLE_COLON) {
		...
		if (lex.next() == TOK_STRING) {
			lex.consume();
		} else if (!type_mode && (lex.next() == TOK_LT ||
					  lex.next() == TOK_DOUBLE_LT)) {
			if (!consume_tt_angle(lex)) return false;
	...
	if (type_mode && lex.next() == TOK_PAREN_OPEN) {
		if (!consume_tt(lex)) return false;
		if (lex.consume_if(TOK_THINARROW)) {
			if (!consume_type(lex)) return false;
		}
	}
	return true;
}
	...
	switch (lex.next()) {
		...
		case TOK_IDENT:
			if (TARGETVER_LEAST_1_29 &&
			    lex.next_tok().ident().name == "dyn")
				lex.consume();
			if (0) case TOK_RWORD_IMPL:
			case TOK_RWORD_DYN:
				lex.consume();
		case TOK_RWORD_CRATE:
		case TOK_RWORD_SUPER:
		...
		case TOK_DOUBLE_COLON:
		case TOK_INTERPOLATED_PATH:
			if (!consume_path(lex, true)) return false;
			if (lex.consume_if(TOK_EXCLAM)) {
				if (lex.next() != TOK_PAREN_OPEN &&
				    lex.next() != TOK_SQUARE_OPEN &&
				    lex.next() != TOK_BRACE_OPEN)
					return false;
				if (!consume_tt(lex)) return false;
			}
			return true;
		...
		case TOK_RWORD_UNSAFE:
			...
			if (lex.next() != TOK_RWORD_FN) return false;
		case TOK_RWORD_FN:
			lex.consume();
			if (lex.next() != TOK_PAREN_OPEN) return false;
			if (!consume_tt(lex)) return false;
			if (lex.consume_if(TOK_THINARROW)) {
				consume_type(lex);
			}
...
bool consume_pat(TokenStreamRO& lex, bool allow_or = true) {
	...
	if (lex.next() == TOK_RWORD_REF || lex.next() == TOK_RWORD_MUT) {
		...
		lex.consume_if(TOK_RWORD_MUT);
		if (!lex.consume_if(TOK_IDENT)) return false;
		if (!lex.consume_if(TOK_AT)) return true;
	}

	if (lex.consume_if(TOK_INTERPOLATED_PATTERN)) return true;
	...
	for (;;) {
		switch (lex.next()) {
			...
			case TOK_RWORD_BOX:
				lex.consume();
				if (!consume_pat(lex, allow_or)) return false;
				if (allow_or && lex.consume_if(TOK_PIPE))
					continue;
				return true;
			case TOK_AMP:
		...
		if (lex.consume_if(TOK_AT)) continue;
		// ... or ..=
		if (lex.consume_if(TOK_TRIPLE_DOT) ||
		    lex.consume_if(TOK_DOUBLE_DOT_EQUAL)) {
			switch (lex.next()) {
				case TOK_IDENT:
				case TOK_RWORD_SUPER:
				case TOK_RWORD_SELF:
		...
		if (allow_or && lex.consume_if(TOK_PIPE)) continue;
		return true;
	}
}
...
bool consume_expr(TokenStreamRO& lex, bool no_struct_lit = false) {
	...
	do {
		...
		switch (lex.next()) {
			...
			case TOK_LT:
			case TOK_DOUBLE_LT:
				if (!consume_path(lex)) return false;
				if (lex.next() == TOK_BRACE_OPEN &&
				    !no_struct_lit)
					consume_tt(lex);
			...
			case TOK_RWORD_UNSAFE:
				lex.consume();
				if (lex.next() != TOK_BRACE_OPEN) return false;
			case TOK_PAREN_OPEN:
			case TOK_SQUARE_OPEN:
			case TOK_BRACE_OPEN:
				consume_tt(lex);
			...
			case TOK_RWORD_FOR:
				lex.consume();
				if (!consume_pat(lex)) return false;
				if (!lex.consume_if(TOK_RWORD_IN)) return false;
				if (!consume_expr(lex, true)) return false;
				if (lex.next() != TOK_BRACE_OPEN) return false;
				if (!consume_tt(lex)) return false;
				break;
			case TOK_RWORD_MATCH:
				lex.consume();
				// TODO: Parse _without_ consuming a struct
				// literal
				if (!consume_expr(lex, true)) return false;
				if (lex.next() != TOK_BRACE_OPEN) return false;
				if (!consume_tt(lex)) return false;
			...
			case TOK_RWORD_IF:
				while (1) {
					...
					if (lex.next() != TOK_BRACE_OPEN)
						return false;
					consume_tt(lex);
					if (lex.next() != TOK_RWORD_ELSE) break;
					lex.consume();
...
bool consume_stmt(TokenStreamRO& lex) {
	...
	if (lex.consume_if(TOK_RWORD_LET)) {
		...
		if (lex.consume_if(TOK_EQUAL)) {
			if (!consume_expr(lex)) return false;
		}
		return true;
	} else {
		if (!consume_expr(lex)) return false;
...
bool consume_vis(TokenStreamRO& lex) {
	...
	if (lex.consume_if(TOK_INTERPOLATED_VIS) ||
	    lex.consume_if(TOK_RWORD_CRATE)) {
		return true;
		...
		// HACK: If the next character is nothing interesting, then
		// force no match?
		// - TODO: Instead, have `:vis` force a deepeer check
		if (lex.next() == TOK_EOF || lex.next() == TOK_PAREN_CLOSE ||
		    lex.next() == TOK_BRACE_CLOSE ||
		    lex.next() == TOK_SQUARE_CLOSE) {
			return false;
		}
...
bool consume_item(TokenStreamRO& lex) {
	...
	// Interpolated items
	if (lex.consume_if(TOK_INTERPOLATED_ITEM)) return true;
	// Macro invocation
	// TODO: What about `union!` as a macro? Needs to be handled below
	if ((lex.next() == TOK_IDENT &&
	     lex.next_tok().ident().name != "union") ||
	    lex.next() == TOK_RWORD_SELF || lex.next() == TOK_RWORD_SUPER ||
	    lex.next() == TOK_DOUBLE_COLON) {
		if (!consume_path(lex)) return false;
		if (!lex.consume_if(TOK_EXCLAM)) return false;
		lex.consume_if(TOK_IDENT);
		bool need_semicolon = (lex.next() != TOK_BRACE_OPEN);
		consume_tt(lex);
		if (need_semicolon) {
			if (!lex.consume_if(TOK_SEMICOLON)) return false;
		}
		return true;
	}
	// Normal items
	if (!consume_vis(lex)) return false;
	if (lex.next() == TOK_RWORD_UNSAFE) lex.consume();
	DEBUG("Check item: " << lex.next_tok());
	switch (lex.next()) {
		case TOK_RWORD_USE:
			// Lazy mode
			while (lex.next() != TOK_SEMICOLON) lex.consume();
			lex.consume();
			break;
		case TOK_RWORD_MOD:
			lex.consume();
		...
		case TOK_RWORD_IMPL:
			lex.consume();
			if (!H::maybe_generics(lex)) return false;
			if (!consume_type(lex)) return false;
			if (lex.consume_if(TOK_RWORD_FOR)) {
				if (!consume_type(lex)) return false;
			}
			if (!H::maybe_where(lex)) return false;
			if (lex.next() != TOK_BRACE_OPEN) return false;
			return consume_tt(lex);
		// type Foo
		case TOK_RWORD_TYPE:
			...
			if (!lex.consume_if(TOK_EQUAL)) return false;
			if (!consume_type(lex)) return false;
			if (!lex.consume_if(TOK_SEMICOLON)) return false;
		...
		case TOK_RWORD_STATIC:
			lex.consume();
			if (!lex.consume_if(TOK_IDENT)) return false;
			if (!lex.consume_if(TOK_COLON)) return false;
			if (!consume_type(lex)) return false;
			if (!lex.consume_if(TOK_EQUAL)) return false;
			if (!consume_expr(lex)) return false;
			if (!lex.consume_if(TOK_SEMICOLON)) return false;
		...
		case TOK_RWORD_CONST:
			lex.consume();
			if (lex.next() == TOK_RWORD_UNSAFE) lex.consume();
			if (lex.next() == TOK_RWORD_EXTERN) lex.consume();
		...
		case TOK_RWORD_EXTERN:
			...
			if (!lex.consume_if(TOK_RWORD_FN)) return false;
			goto fn;
		case TOK_RWORD_FN:
			lex.consume();
		fn:
...
bool consume_from_frag(TokenStreamRO& lex, MacroPatEnt::Type type) {
	...
	switch (type) {
		...
		case MacroPatEnt::PAT_LOOP:
			BUG(Span(),
			    "Encountered " << type << " in consume_from_frag");
			;
		case MacroPatEnt::PAT_BLOCK:
			if (lex.next() == TOK_BRACE_OPEN) {
				return consume_tt(lex);
...
unsigned int Macro_InvokeRules_MatchPattern(
	...
	for (size_t i = 0; i < rules.m_rules.size(); i++) {
		...
		auto arm_stream =
		    MacroPatternStream(rules.m_rules[i].m_pattern);

		bool fail = false;
		for (;;) {
			const auto pos = arm_stream.cur_pos();
			const auto& pat = arm_stream.next();
			// NOTE: The positions seen by this aren't fully
			// sequential, as `next` steps over jumps/loop control
			// ops
			DEBUG("Arm " << i << " @" << pos << " " << pat);
			if (pat.is_End()) {
				if (lex.next() != TOK_EOF) fail = true;
				break;
				...
				for (const auto& check : e->ents) {
					if (check.ty !=
					    MacroPatEnt::PAT_TOKEN) {
						if (!consume_from_frag(
							lc, check.ty)) {
							rv = false;
							break;
						}
		...
		if (!fail) {
			matches.push_back(
			    ::std::make_pair(i, arm_stream.take_history()));
			DEBUG(i << " MATCHED");
		} else {
			DEBUG(i << " FAILED");
			fail_pos.push_back(
			    std::make_pair(lex.position(), lex.next()));
		}
	}
	...
	if (matches.size() == 0) {
		...
		SET_MODULE(lex, mod);
		auto arm_stream =
		    MacroPatternStream(rules.m_rules[i].m_pattern, &history);
		...
		for (;;) {
			...
			if (pat.is_End()) {
				...
				BUG(sp,
				    "Unexpected If pattern during final "
				    "matching - "
					<< pat);
			} else if (const auto* e = pat.opt_ExpectTok()) {
				...
				capture_info.push_back(
				    Capture{e->idx, arm_stream.get_loop_iters(),
					    cap_idx});
			} else {
				// Unreachable.
			}
		...
		for (const auto& cap : capture_info) {
			bound_tts.insert(cap.binding_idx, cap.iterations,
					 mv$(captures[cap.cap_idx]));
		}
		...
		if (const auto* e = ent_ptr->opt_NamedValue()) {
			switch (*e & ~NAMEDVALUE_VALMASK) {
				...
				case NAMEDVALUE_TY_IGNORE:
					...
					bound_tts.inc_count(
					    Span(), state.iterations(),
					    *e & NAMEDVALUE_VALMASK);
					break;
	...
	TU_MATCH_HDRA( (ent), {)
        TU_ARMA(Token, e) {
				switch (e.type()) {
					case TOK_IDENT:
					case TOK_LIFETIME: {
						// Rewrite the hygiene of an
						// ident such that idents in the
						// macro explicitly are unique
						// for each expansion
			...
			TU_ARMA(NamedValue, e) {
				switch (e & ~NAMEDVALUE_VALMASK) {
					default:
						BUG(this->point_span(),
						    "Unknown macro metavar - 0x"
							<< std::hex << e);
					case NAMEDVALUE_TY_COUNT: {  // `${count(VarName)}`
						auto count =
						    m_mappings.get_variable_count(
							this->point_span(),
							m_state.iterations(),
							e & NAMEDVALUE_VALMASK);
						return Token(U128(count),
							     CORETYPE_ANY);
						break;
					}
					case NAMEDVALUE_TY_IGNORE: {  // `${ignore(VarName)}`
						auto* frag = m_mappings.get(
						    this->point_span(),
						    m_state.iterations(),
						    e & NAMEDVALUE_VALMASK);
						ASSERT_BUG(
						    this->point_span(), frag,
						    "Cannot find '"
							<< (e &
							    NAMEDVALUE_VALMASK)
							<< "' for "
	...
	while (m_offsets.size() > 0) {
	    ...
	    TU_MATCH_HDRA( (ent), {)
            TU_ARMA(Token, e) {
				...
				TU_ARMA(NamedValue, e) { return &ent; }
				TU_ARMA(Loop, e) {
					assert(
					    !e.controlling_input_loops.empty());
					unsigned int num_repeats =
					    m_mappings.get_loop_repeats(
						Span(), m_iterations,
						*e.controlling_input_loops
						     .begin());
					for (auto loop_ident :
					     e.controlling_input_loops) {
						if (loop_ident ==
					...
					DEBUG("Looping "
					      ...
					      << e.controlling_input_loops
					      << "}");
					// 2. If it's going to repeat, start the
					// loop
					if (num_repeats > 0) {
						m_offsets.push_back(
						    {0, 0, num_repeats});
	    ...
	    // Fall through for loop
			...
			DEBUG("layer = " << layer << ", m_iterations = "
					 << m_iterations);
			auto& cur_ofs = m_offsets.back();
			DEBUG("Layer #" << layer
					<< " Cur: " << cur_ofs.loop_index
					<< ", Max: " << cur_ofs.max_index);
			if (cur_ofs.loop_index + 1 < cur_ofs.max_index) {
				...
				if (loop_layer.as_Loop().joiner.type() !=
				    TOK_NULL) {
					DEBUG("- Separator token = "
					      << loop_layer.as_Loop().joiner);
					return &loop_layer;
				}
